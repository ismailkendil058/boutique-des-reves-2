...
function LoginPage() {
  ...
  const loadEmployees = useStore((s) => s.loadEmployees);

  // Always load employees on mount to ensure fresh data from Supabase
  useEffect(() => {
    loadEmployees();
  }, [loadEmployees]);
  ...
  useEffect(() => {
    if ((!selectedId || !employees.some((e) => e.id === selectedId)) && employees.length > 0) {
